  ...
  return (
    <>
      ...
      >
        ...
        >
          {!collapsed && (
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-br from-slate-700 via-purple-600 to-slate-700 rounded-lg flex items-center justify-center shadow-lg">
          ...
          <button
            ...
            className="p-1.5 sm:p-2 rounded-lg hover:bg-purple-800/20 transition-colors text-purple-300 flex-shrink-0"
        ...
        {/* New Chat Button */}
        <div
          className={`${collapsed ? "px-2" : "px-3 sm:px-4"} py-3 flex-shrink-0`}
        >
          {collapsed ? (
            <button
            ...
            <button className="w-full bg-gradient-to-r from-purple-800/30 via-purple-700/40 to-purple-800/30 hover:from-purple-700/40 hover:via-purple-600/50 hover:to-purple-700/40 border-purple-700/50 text-purple-200 border rounded-lg py-1 px-3 sm:px-4 font-medium transition-all duration-200 flex items-center justify-center gap-2 text-sm sm:text-base">
            ...
            <div className="flex flex-col gap-1 rounded-lg p-1">
              {navigationItems.map((item) => (
                <button
                  ...
                  className={`flex items-center border border-purple-800 gap-2 px-2.5 sm:px-3 py-2 rounded-md text-sm font-medium transition-all duration-200 ${
        ...
        {/* Divider */}
        <hr className="border-t border-purple-600/50 mx-3 sm:mx-4 my-4 flex-shrink-0" />
            ...
            {filteredConversations.map((conv) => (
              ...
              >
                    ...
                    <div className="flex items-center justify-between w-full min-w-0">
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                        <div
                          ...
                          className="p-1 sm:p-1.5 hover:bg-purple-700/40 rounded-md transition-colors cursor-pointer"
                          title="More options"
                        >
                          <MoreVertical className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-purple-300" />
                        </div>
                      </div>
                    </div>
                  )}
                ...
                {showContextMenu === conv.id && !collapsed && (
                  <div className="absolute right-2 top-10 bg-slate-800/95 border-purple-700/50 backdrop-blur-md border rounded-lg shadow-xl z-20 min-w-36 sm:min-w-40 py-1">
                    <button
                      className="w-full flex items-center gap-2 sm:gap-3 px-2.5 sm:px-3 py-2 text-xs sm:text-sm hover:bg-purple-700/30 transition-colors text-purple-200"
                      onClick={() => setShowContextMenu(null)}
                    >
                    ...
                    <button
                      className="w-full flex items-center gap-2 sm:gap-3 px-2.5 sm:px-3 py-2 text-xs sm:text-sm hover:bg-red-900/30 transition-colors text-red-400"
                      onClick={() => setShowContextMenu(null)}
                    >
                ...
                {collapsed && hoveredConv === conv.id && (
                  <div className="absolute left-16 top-1/2 transform -translate-y-1/2 bg-slate-800/95 text-white border-purple-700/50 px-2.5 sm:px-3 py-2 rounded-lg shadow-lg z-50 min-w-56 sm:min-w-64 border pointer-events-none backdrop-blur-md">
